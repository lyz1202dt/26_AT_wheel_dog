...
class Cross_StepState : public BaseState<Robot> {
public:
    Cross_StepState(Robot* robot);
    bool enter(Robot* robot, const std::string& last_status) override;
    std::string update(Robot* robot) override;

private:
    int cross_step_stage{-1};
    rclcpp::Time cross_step_stage_time;
    Vector3D step_lf_foot_pos{0,0,0}, step_rf_foot_pos{0,0,0}, step_lb_foot_pos{0,0,0}, step_rb_foot_pos{0,0,0};
