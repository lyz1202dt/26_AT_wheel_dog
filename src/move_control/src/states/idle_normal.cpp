...
idle_normal::idle_normal(Robot* robot)
    : BaseState<Robot>("idle_normal") {
    (void)robot;
}

bool idle_normal::enter(Robot* robot, const std::string& last_status) {
    (void)robot;
    (void)last_status;
    return true;
}
...
std::string idle_normal::update(Robot* robot) {
    // TODO:更新状态
    ...
    if ((!trajectory_calced)) {
        if(robot->driver_or_sim == driver)
            robot->node_->set_parameters({
                rclcpp::Parameter("lf_grivate", 32.0),
                rclcpp::Parameter("rf_grivate", 32.0),
                rclcpp::Parameter("lb_grivate", 40.0),
                rclcpp::Parameter("rb_grivate", 40.0)
    
            });
        else 
            robot->node_->set_parameters({
                    rclcpp::Parameter("lf_grivate", 24.0),
                    rclcpp::Parameter("rf_grivate", 24.0),
                    rclcpp::Parameter("lb_grivate", 28.0),
                    rclcpp::Parameter("rb_grivate", 28.0)
        ...
        lb_leg_step.update_support_trajectory(robot->lb_joint_pos, Vector3D(robot->lb_joint_pos(0), 0.79, -0.14), 3.0);
        rb_leg_step.update_support_trajectory(robot->rb_joint_pos, Vector3D(robot->rb_joint_pos(0), -0.79, 0.14), 3.0);
    }
